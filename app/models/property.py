...
class Property(Base):
    ...
    # Pricing - Using Numeric for accurate currency handling
    ...
    # Details - Using Numeric for area measurements
    ...
    # Geolocation - FIXED: Using Numeric for proper decimal storage
    latitude = Column(Numeric(10, 8), nullable=True)  # e.g., 44.42681234
    longitude = Column(Numeric(11, 8), nullable=True)  # e.g., 26.10250000
    ...
    # Note: PropertyPriceHistory relationship removed to avoid circular import
    # If you need price history tracking, add it in app/models/alerts.py:
    ...
    def set_coordinates(self, lat: float, lng: float):
        ...
        """
        # Store as Numeric (proper decimal precision)
        self.latitude = lat
        self.longitude = lng
    ...
    def calculate_price_per_sqm(price: float, area: float) -> float:
        ...
        Returns:
            Price per sqm rounded to 2 decimal places
        """
        if area > 0:
